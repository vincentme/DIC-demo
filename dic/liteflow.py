...
class Network(torch.nn.Module):
    def __init__(self):
        ...
        self.load_state_dict(torch.load('{}/network-'.format(os.path.dirname(os.path.realpath(__file__))) + arguments_strModel + '.pytorch'))
    # end
...
def estimate(img1, img2):
    ...
    if len(img1.shape) == 2:
        img1 = np.repeat(np.expand_dims(img1, 0), 3, 0)
        img2 = np.repeat(np.expand_dims(img2, 0), 3, 0)
    tensorFirst = torch.FloatTensor(img1 * (1.0 / 255.0))
    tensorSecond = torch.FloatTensor(img2 * (1.0 / 255.0))
    ...
    opticalflow = tensorFlow[0, :, :, :].cpu().numpy()
    return opticalflow[0], opticalflow[1]
